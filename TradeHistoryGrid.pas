unit TradeHistoryGrid;

interface

uses
  Vcl.Grids, System.Generics.Collections, System.SysUtils, Vcl.Graphics,
  System.Variants, Math, Common; // <-- For SecurityColors

type
  TTradeHistoryRec = record
    DateTime: TDateTime;
    TypeID: Integer;
    BuyStationID: Integer;
    Quantity: Integer;
    BuyPrice: Double;
    SellStationID: Integer;
    SellPrice: Double;
    SalesTax: Double;
  end;
  ...
  public
    function TypeIDToName(TypeID: Integer): string; virtual; abstract;
    function StationIDToName(StationID: Integer): string; virtual; abstract;
    function TypeIDToVolume(TypeID: Integer): Double; virtual; abstract;
    function StationIDToSecurity(StationID: Integer): Double; virtual; abstract;
  end;

procedure OutputTradeHistoryToGrid(
  Grid: TStringGrid;
  const TradeHistory: TDictionary<Integer, TTradeHistoryRec>;
  RegionManager: TRegionManager;
  var CellColors: array of array of TColor;
  SortCol: Integer;
...
function FmtFloat(f: Double; decs: Integer = 0): string;
var
  fmt: string;
begin
  fmt := '0';
...
begin
  ...
  try
    ...
    begin
      ...
      Grid.Cells[8, Row] := FmtFloat(NetBuy, 2);
      Grid.Cells[9, Row] := FmtFloat(NetSell, 2);
      Grid.Cells[10, Row] := FmtFloat(SalesTax, 2);
      Grid.Cells[11, Row] := FmtFloat(NetProfit, 2);
      Grid.Cells[12, Row] := Format('%.2f%%', [(NetProfit / NetBuy * 100)]);
    end;
  finally
    TradeList.Free;
  end;
